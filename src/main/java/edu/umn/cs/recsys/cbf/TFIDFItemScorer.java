...
public class TFIDFItemScorer extends AbstractItemScorer {
     ...
     *
     * @param uedao The user-event DAO, so we crn fetch a user's ratings when scoring items for them.
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        for (Long item: items) {
            ...
            for(Long tag: iv.keySet()){
                if(userVector.get(tag)!=null)
                    score+=iv.get(tag)*userVector.get(tag);
            }
            score = score/((Vectors.euclideanNorm(iv))*Vectors.euclideanNorm(userVector));
