...
public class TFIDFModelBuilder implements Provider<TFIDFModel> {
    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (String tagName : itemTagDAO.getItemTags(item)) {
                Double value = work.get(tagIds.get(tagName));
                if (value == null)
                    work.put(tagIds.get(tagName), 1);
                else
                    work.put(tagIds.get(tagName), new Double(value + 1.0));// finish TF vector for item

            }
            for (Long longTag : work.keySet()) {
                //logger.info("tagid : {}", longTag);
                if (docFreq.containsKey(longTag))
                    docFreq.put(longTag, Double.valueOf(docFreq.get(longTag) + 1));
                else
                    docFreq.put(longTag, 1);
            }
        ...
        for (Map.Entry<Long, Long2DoubleMap> entry : itemVectors.entrySet()) {
            ...
            for (Long longTag : tv.keySet()) {
                tv.put(longTag, Double.valueOf(tv.get(longTag) * docFreq.get(longTag)));
            }
            // TODO Convert this TF vector into a TF-IDF vector
            double norm = Vectors.euclideanNorm(tv);
            for (Long longTag : tv.keySet()) {
                tv.put(longTag, Double.valueOf(tv.get(longTag) / norm));
            }
